...
QNetworkInterface NetUtils::getPreferedInterface() {
    ...
    if(addresses.contains(NetUtils::ClassC)){
        return addresses[NetUtils::ClassC];
    }
    else if(addresses.contains(NetUtils::ClassA)){
        return addresses[NetUtils::ClassA];
    }
    else if(addresses.contains(NetUtils::ClassB)){
        return addresses[NetUtils::ClassB];
    }
...
QList<QHostAddress> NetUtils::getGateways() {
    ...
    if(dwRetVal != NO_ERROR) {
        qWarning() << "GetAdaptersInfo failed with error: " << dwRetVal;
    }
    else {
